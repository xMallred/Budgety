...
export var BudgetTable = {
  ...
  addDateRow: function (budgetTable) {
    ...
    if (payFrequency === "biWeekly") {
      ...
      for (let i = 0; i < budgetLength; i++) {
        ...
        var formattedDate = ('0' + (date.getMonth() + 1)).slice(-2) + "/" + ('0'
    ...
    if (payFrequency === "semiMonthly") {
      ...
      for (let i = 0; i < budgetLength; i++) {
        ...
        var formattedDate = ('0' + (payDate.getMonth() + 1)).slice(-2) + "/"
            + ('0' + (payDate.getDate())).slice(-2) + "/"
            + payDate.getFullYear();
        this.addHeaderCell(dateRow, formattedDate);
  ...
  addExpenseRows: function (budgetTable) {
    ...
    let i = 1;
    while (document.getElementById("expense" + i + "Amount") != null
    && !document.getElementById("expense" + i + "Frequency") != null) {
      var expenseAmount = document.getElementById("expense" + i
          + "Amount").value;
      var expenseFrequency = document.getElementById("expense" + i
          + "Frequency").value;
      if (expenseAmount !== "") {
        ...
        for (let j = 0; j < budgetLength; j++) {
          if (payFrequency === "monthly") {
            ...
            if (expenseFrequency === "onceAMonth") {
              ...
              //Date of expense
              var dayOfExpense = document.getElementById("expense" + i
                  + "Date").value;
              ...
              if (expenseDate.getTime() >= payDay && expenseDate.getTime()
                  < LastDayOfPayCycle.getTime()) {
                this.addCell(expenseRow, "-$" + expenseAmount);
                currentBalances[j] = currentBalances[j] + parseFloat(
                    expenseAmount);
              }
              else {
  ...
  addRemainingBalanceRow: function (budgetTable) {
    ...
    for (var i = 0; i < currentBalances.length; i++) {
      this.addCell(balanceRow, '$' + (parseFloat(payCheck)
          - currentBalances[i]));
    }
