...
export var DebtTable = {
    ...
    setFirstRow: function() {
        ...
        this.addHeaderCell(row, "Starting Balance");
        this.addHeaderCell(row, "Balance as of: " + budgetTable.firstElementChild.lastChild.innerHTML)
        this.addHeaderCell(row, "Amount Paid as of: " + budgetTable.firstElementChild.lastChild.innerHTML);
        this.addHeaderCell(row, "Interest accrued as of: " + budgetTable.firstElementChild.lastChild.innerHTML)
    ...
    setDebts: function() {
        ...
        while(document.getElementById("debt" + i + "Name") != null && !document.getElementById("debt" + i + "Amount") != null && !document.getElementById("debt" + i + "InterestPercent") != null){
            ...
            var debtInterestPercent = document.getElementById("debt" + i + "InterestPercent").value;
            var principalAmount = document.getElementById("debt" + i + "Principal").value;
            if(debtName != "" || debtAmount != "" || debtInterestPercent != "") {
                ...
                var debtBeingPaidRowNumber = this.debtBeingPaidRow(budgetTableRows, debtName);
                ...
                for(let z = 1; z < datesRowChildren.length; z++) {
                    ...
                    if(daysPassed != 0 && debtInterestPercent != "" && debtInterestPercent != 0) {
                        var dailyInterestPercent = (parseFloat(debtInterestPercent) / 100) / 365;
                        var dailyCharge = parseFloat(remainingPrincipal) * parseFloat(dailyInterestPercent);
                        ...
                        totalInterestAccrued = parseFloat(totalInterestAccrued) + parseFloat(interestAccrued);
                        remainingBalance = parseFloat(remainingBalance) + parseFloat(interestAccrued);
                    ...
                    if(debtBeingPaidRowNumber != -1) {
                        ...
                        if(budgetTableRowChildren[z].innerHTML != "") {
                            ...
                            console.log("Remaining Balance after payment: " + remainingBalance);
    ...
    daysPassed: function(datesRowChildren, z) {
        ...
        var previousDate = new Date(previousDateParts[2], previousDateParts[0] - 1, previousDateParts[1]);
        
        var daysPassed = Math.round(currentDate.getTime() - previousDate.getTime()) / (1000 * 60 * 60 * 24);
